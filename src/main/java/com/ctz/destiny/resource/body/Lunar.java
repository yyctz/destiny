...
public class Lunar {
    ...
    public static List<JieQi> Alljieqi(int year) {

        List<JieQi> jieqi = new ArrayList<>();
        jieqi.addAll(JieQiUtil.getYearJieQiList(year-1));
        jieqi.addAll(JieQiUtil.getYearJieQiList(year));
        jieqi.addAll(JieQiUtil.getYearJieQiList(year+1));
        return jieqi;
    }
    ...
    public static int[] getnearsolarTerm(int year, Date date) {
        List<JieQi> jieqi = Alljieqi(year);

        int[] returnValue = new int[2];
        for (int i = 0; i < jieqi.size(); i++) {
            if (date.getTime() > jieqi.get(i).getJieQiDate().getTime()) {
                continue;
            }
            if (i % 2 == 0) {//只管气
                returnValue[0] = i - 2;
    ...
    private void getCyclicalData() {
        ...
        this.solarMonth = this.solar.get(Calendar.MONTH)+1;
